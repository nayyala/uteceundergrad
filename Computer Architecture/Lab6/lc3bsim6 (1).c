...
void MEM_stage() {
	...
	if (Get_DCACHE_RW(&PS.MEM_CS) == 0) {
		...
		if ((PS.MEM_ADDRESS & 0x01) && (Get_DATA_SIZE(&PS.MEM_CS) == 0)) {
			memw0 = 0;
			memw1 = 1;
		}
		else if (((PS.MEM_ADDRESS & 0x01) == 0) && (Get_DATA_SIZE(&PS.MEM_CS) == 0)) {
			memw0 = 1;
			memw1 = 0;
		}
		else if (((PS.MEM_ADDRESS & 0x01) == 0) && (Get_DATA_SIZE(&PS.MEM_CS) == 1)) {
			memw0 = 1;
			memw1 = 1;
		}
	}
	...
	if (Get_DATA_SIZE(&PS.MEM_CS) == 1) {
		write_dcache = PS.MEM_ALU_RESULT;
	}
	else if (!Get_DATA_SIZE(&PS.MEM_CS) && ma0) {
		write_dcache = (PS.MEM_ALU_RESULT & 0xFF00) + (PS.MEM_ALU_RESULT >> 8);
	}
	else if (!Get_DATA_SIZE(&PS.MEM_CS) && !ma0) {
		write_dcache = ((PS.MEM_ALU_RESULT & 0xFF) << 8) + (PS.MEM_ALU_RESULT & 0xFF);
	}
	
	/*BR Logic Block*/
	int irbits = (PS.MEM_IR & 0x0E00) >> 9;
	if (PS.MEM_V != 0) {
	...
	/*Cache access*/
	if (v_dcache_en) {
		dcache_access(PS.MEM_ADDRESS, &read_dcache, write_dcache, &dcache_r, memw0, memw1);
	}
	...
	/*SR Data Logic*/
		if (!ma0 && Get_DATA_SIZE(&PS.MEM_CS)) {
			NEW_PS.SR_DATA = read_dcache;
		}
		else if (ma0 && !Get_DATA_SIZE(&PS.MEM_CS)) {
			NEW_PS.SR_DATA = signextend(((read_dcache & 0xFF00) >> 8),8)&0xFFFF;
		}
		else if (!ma0 && !Get_DATA_SIZE(&PS.MEM_CS)) {
			NEW_PS.SR_DATA = signextend((read_dcache & 0xFF),8)&0xFFFF;
		}

	trap_pc = Low16bits(NEW_PS.SR_DATA);
	...
	other SR latches. */
		...
		NEW_PS.SR_DRID = NEW_PS.MEM_DRID;
...
void AGEX_stage() {
	...
	case 0:
		...
		break;
	case 1:
		offsetmux = PS.AGEX_IR & 0x3F;
		offsetmux = signextend(offsetmux, 6);
		break;
	case 2:
		offsetmux = PS.AGEX_IR & 0x1FF;
		offsetmux = signextend(offsetmux, 9);
		break;
	case 3:
		offsetmux = PS.AGEX_IR & 0x7FF;
		offsetmux = signextend(offsetmux, 11);
		break;
	}

	offsetmux = offsetmux << 1;
	memadd = memadd + offsetmux;

	switch (Get_ADDRESSMUX(&PS.AGEX_CS)) {
	case 0:
		memadd = (PS.AGEX_IR & 0xFF) << 1;
		break;
	case 1:
		memadd = memadd;
		break;
	...
	case 0:
		SR2MUXout = PS.AGEX_SR2;
		break;
	case 1:
		SR2MUXout = signextend((PS.AGEX_IR & 0x1F), 5);
		break;
	}

	switch (Get_ALUK(&PS.AGEX_CS)) {
	case 0:
		memalu = SR2MUXout + PS.AGEX_SR1;
		break;
	case 1:
		memalu = SR2MUXout & PS.AGEX_SR1;
		break;
	...
	offsetmux = PS.AGEX_IR & 0x0F;
	switch ((PS.AGEX_IR & 0x030) >> 4) {
	case 0:
	case 2:
		offsetmux = PS.AGEX_SR1 << offsetmux;
		break;
	case 1:
		offsetmux = (PS.AGEX_SR1 & 0xFFFF) >> offsetmux;
		break;
	case 3:
		offsetmux = signextend(PS.AGEX_SR1,16) >> offsetmux;
		break;
	}
	...
	if (LD_MEM) {
		/* Your code for latching into MEM latches goes here */
			...
			NEW_PS.MEM_ADDRESS = Low16bits(memadd);
...
void DE_stage() {
	...
	CONTROL_STORE_ADDRESS = ((PS.DE_IR&0xF000)>>10) + ((PS.DE_IR&0x0800)>>10) + ((PS.DE_IR&0x0020)>>5);  /* You need to implement the logic to
	...
	Inputs: SR1, SR2, DE.V, SR1.NEEDED, SR2NEEDED
	DE.BR.OP, V.AGEXLdCC, V.MEMLDcc, V.SRLDcc,
	V.SRLDreg, VMEMLDreg, VAgexLdreg, SRDRID,
	Mem.DRID, AGEXDrid
	Output: DEPStall*/

	/*REWRITE THIS TO BE YOUR OWN CODE*/

	int SR1 = ((PS.DE_IR & 0x1C0) >> 6);
	int SR2 = (PS.DE_IR & 0x2000) ? ((PS.DE_IR & 0x0E00) >> 9) : (PS.DE_IR & 0x07);
	if (!PS.DE_V)
		dep_stall = 0;
	else if (Get_SR1_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && ((v_agex_ld_reg && (PS.AGEX_DRID == SR1)) || (v_mem_ld_reg && (PS.MEM_DRID == SR1)) || (v_sr_ld_reg && (sr_reg_id == SR1))))
		dep_stall = 1;
	else if (Get_SR2_NEEDED(CONTROL_STORE[CONTROL_STORE_ADDRESS]) && ((v_agex_ld_reg && (PS.AGEX_DRID == SR2)) || (v_mem_ld_reg && (PS.MEM_DRID == SR2)) || (v_sr_ld_reg && (sr_reg_id == SR2))))
		dep_stall = 1;
	...
	/*AgexV Logic*/
	LD_AGEX = !mem_stall;
	/*!dep_stall && !(Get_DE_BR_STALL(&PS) && Get_MEM_BR_STALL(&PS)) && */
...
void FETCH_stage() {
	...
	int instr = 0;
		...
		else if ((!icache_r) || (dep_stall) || (v_agex_br_stall&&Get_AGEX_BR_STALL(&PS.AGEX_CS)) || (v_de_br_stall) || (mem_stall)) {
			ld_pc = 0;
		}
		else {
			ld_pc = 1;
		...
		if (ld_de) {
			NEW_PS.DE_IR = instr;
			NEW_PS.DE_NPC = PC + 2;
			if ((v_agex_br_stall) || (v_mem_br_stall) || (icache_r != 1) || (v_de_br_stall)) {
				NEW_PS.DE_V = 0;
			}
			else {
				NEW_PS.DE_V = 1;
